...
object OpFallingBlock : SpellAction {
	...
	override fun execute(args: List<Iota>, env: CastingEnvironment): SpellAction.Result {
		...
		return SpellAction.Result(
			Spell(pos),
			(HexalConfig.Server.FALLING_BLOCK_COST.get() * 10000.0).toLong(),
	...
	private data class Spell(val v: Vec3) : RenderedSpell {
		override fun cast(env: CastingEnvironment) {
			...
			if ((
				...
				&& canSilkTouch(env.world, pos, blockstate, tier.attackDamageBonus.toInt(), env.castingEntity)
			) {
		...
		fun canSilkTouch(level: ServerLevel, pos: BlockPos, state: BlockState, harvestLevel: Int, owner: Entity?): Boolean {
			...
			// I'm not sure how to fix this, so this is temporarily broken.
