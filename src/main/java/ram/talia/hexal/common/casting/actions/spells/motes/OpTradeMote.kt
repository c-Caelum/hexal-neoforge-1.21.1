...
object OpTradeMote : VarargConstMediaAction {
    override val mediaCost: Long
        get() = Hexal.toMediaFromDust(HexalConfig.Server.TRADE_ITEM_COST.get());
    ...
    override fun execute(args: List<Iota>, argc: Int, userData: CompoundTag, env: CastingEnvironment): List<Iota> {
        val villager = args.getVillager(0, argc, env.world)
        ...
        do {
            ...
            val toTradeItemStacks = toTradeItemIotas.map { it.record?.toStack(it.record?.maxStackSize()?.toInt() ?: it.item.defaultMaxStackSize) }
            ...
            if (merchantoffer.take(toTrade0, toTrade1) || merchantoffer.take(toTrade1, toTrade0)) {
                ...
                else if (outRecord.item == merchantoffer.result.item && outRecord.components == merchantoffer.result.components)
                    outRecord.count += merchantoffer.result.count
                else {
                    Hexal.LOGGER.warn("trade resulted in different Item result in different iterations of the loop, aghhhhh")
                    break
                }

                toTradeItemIotas.getOrNull(0)?.removeItems(toTradeItemIotas[0].record!!.maxStackSize() - toTrade0.count)
                toTradeItemIotas.getOrNull(1)?.removeItems(toTradeItemIotas[1].record!!.maxStackSize() - toTrade1.count)
            }
