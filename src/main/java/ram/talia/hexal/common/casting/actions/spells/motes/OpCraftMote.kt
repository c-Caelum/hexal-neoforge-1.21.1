...
object OpCraftMote : UserDataConstMediaAction {
    ...
    override val mediaCost: Long
        get() = Hexal.toMediaFromDust(HexalConfig.Server.CRAFT_ITEM_COST.get())
    ...
    override fun execute(args: List<Iota>, userData: CompoundTag, env: CastingEnvironment): List<Iota> {
        ...
        val craftingInput = CraftingInput.of(3,3, List<ItemStack>(9) { idx -> griddedIotas[idx]!!.record!!.toStack() });

        val (itemResult, remainingItems) = getCraftResult(craftingInput, env) ?: return emptyList<Iota>().asActionResult
    ...
    internal fun getCraftResult(container: CraftingInput, env: CastingEnvironment): Pair<ItemStack, List<ItemStack>>? {
        val recman = env.world.recipeManager
        val recipes = recman.getAllRecipesFor(RecipeType.CRAFTING)
        val recipe = (recipes.find { it.value.matches(container, env.world) } ?: return null).value
