...
object MediafiedItemManager {
    ...
    @JvmStatic
    fun getComponents(index: Index): PatchedDataComponentMap? = access(index)?.components

    @JvmStatic
    fun setComponents(index : Index, components: PatchedDataComponentMap) {
    ...
    fun templateOff(index: Index, stack: ItemStack, count: Long?) {
        ...
        val datamap : PatchedDataComponentMap = PatchedDataComponentMap.fromPatch(stack.components, stack.componentsPatch).copy();
        record.components = datamap
    ...
    // empty because I can't properly make the codec in Kotlin.
