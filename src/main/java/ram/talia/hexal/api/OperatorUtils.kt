...
fun List<Iota>.getVillager(idx: Int, argc: Int = 0, level : ServerLevel): Villager {
...
fun List<Iota>.getBlockPosOrItemEntityOrItem(idx: Int, argc: Int = 0, level : ServerLevel): Anyone<BlockPos, ItemEntity, MoteIota> {
    val x = this.getOrElse(idx) { throw MishapNotEnoughArgs(idx + 1, this.size) }
    val out = when (x) {
        is Vec3Iota -> Anyone.first<BlockPos, ItemEntity, MoteIota>(BlockPos.containing(x.vec3))
        is EntityIota -> {
            if (x.getEntity(level).isRemoved)
                null
            else
                (x.getEntity(level) as? ItemEntity)?.let { Anyone.second(it) }
        }
...
fun List<Iota>.getItemEntityOrItemFrame(idx: Int, argc: Int = 0, level : ServerLevel): Either<ItemEntity, ItemFrame> {
    val x = this.getOrElse(idx) { throw MishapNotEnoughArgs(idx + 1, this.size) }
    if (x is EntityIota) {
        val e = x.getEntity(level)
        if (e.isRemoved)
...
fun List<Iota>.getMoteOrItemStackOrItemEntity(idx: Int, argc: Int = 0, level : ServerLevel): Anyone<MoteIota, ItemStack, Entity>? {
    val x = this.getOrElse(idx) { throw MishapNotEnoughArgs(idx + 1, this.size) }
    if (x is MoteIota)
